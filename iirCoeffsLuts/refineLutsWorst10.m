function refine_lut_worst10()

clc;
fprintf('Refining worst 10 LUT entries...\n');
...
topN = min(10,length(idx));
worst_idx = idx(1:topN);
...
w_grid = logspace(log10(w_min),log10(pi),num_points)';
...
for k = 1:topN
  i = worst_idx(k);
    ...
    fprintf('\n============================================================\n');
    fprintf('Recomputing row %d  | idx = [%d %d %d %d]\n', ...
        i, dims(i,1), dims(i,2), dims(i,3), dims(i,4));
    fprintf('fc=%.16g, Q=%.16g, gain_db=%.16g, stages=%.16g\n', ...
        fc, Q, gain_db, stages);
    fprintf('old error = %.9f dB\n', old_rmse);
    ...
    %% ============================================================
    % same algorithm as original generator
    %% ============================================================

    order = 4;
    results = struct([]);

    % ---------- method 1: spectral-irls ----------
    ...
    tmp = evaluate_method(sprintf('spectral-irls (a=%.4f)', aopt1), ...
        b1, a1, R_target, w_grid, fs);
    results = tmp;

    % ---------- method 2: minphase-invfreqz ----------
    ...
    tmp = evaluate_method(sprintf('minphase-invfreqz (a=%.4f)', aopt2), ...
        b2, a2, R_target, w_grid, fs);
    results(end+1) = tmp;

    % ---------- method 3: minphase-stmcb ----------
    ...
    tmp = evaluate_method(sprintf('minphase-stmcb (a=%.4f)', aopt3), ...
        b3, a3, R_target, w_grid, fs);
    results(end+1) = tmp;
    ...
    all_rmse = [results.rmse_db];
    [new_rmse, idx_best] = min(all_rmse);
    best = results(idx_best);

    delta_db = old_rmse - new_rmse;

    fprintf('best method = %s\n', best.name);
    fprintf('new  error  = %.9f dB\n', new_rmse);
    fprintf('reduction   = %.9f dB\n', delta_db);

    %% ============================================================
    % replace only if improved
    %% ============================================================
    ...
    if new_rmse < old_rmse
        ...
        fprintf('status      = SKIPPED (no improvement)\n');
    end
...
newdata = [dims proto err bcoef acoef];

%% ============================================================
% sort by index (保持 LUT 顺序)
%% ============================================================

[~,ord] = sortrows(dims,[1 2 3 4]);
...
fprintf('Finished refining worst 10\n');
...
function [a_best, best_score, b_best, a_best_tf, best_eval] = optimize_warp_parameter_binary( ...
    ...
    n_coarse = 500;     % first-stage global mesh
