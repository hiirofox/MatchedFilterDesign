function rz2iir_analyze_error_and_export()
    clc;
    warning('off', 'all');
    ...
    in_file  = 'iirluts.txt';
    out_file = 'iirluts2.txt';

    if ~exist(in_file, 'file')
        error('Input file not found: %s', in_file);
    end

    fprintf('Reading LUT from %s ...\n', in_file);
    ...
    fid = fopen(in_file, 'r');
    if fid < 0
        error('Cannot open file: %s', in_file);
    end
    ...
    while true
        ...
        % 预期格式：
        % 4 idx + 4 proto + method string + 5 b + 5 a
        % 其中 method string 可能带空格，所以用 regexp 抓头尾，中间当字符串
        expr = ['^\s*' ...
            '(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+' ...                         % 4 dims
            '([+\-]?\d*\.?\d+(?:[eE][+\-]?\d+)?)\s+' ...                   % fc
            '([+\-]?\d*\.?\d+(?:[eE][+\-]?\d+)?)\s+' ...                   % Q
            '([+\-]?\d*\.?\d+(?:[eE][+\-]?\d+)?)\s+' ...                   % gain_db
            '([+\-]?\d*\.?\d+(?:[eE][+\-]?\d+)?)\s+' ...                   % stages
        ...
        vals = cellfun(@str2double, tok([1:8, 10:19]));
    ...
    if n == 0
        error('No valid rows parsed from %s', in_file);
    end

    % ============================================================
    % Recompute error for every row
    % ============================================================
    fprintf('Recomputing errors ...\n');

    for k = 1:n
        fc      = rows(k).fc;
        Q       = rows(k).Q;
        gain_db = rows(k).gain_db;
        stages  = rows(k).stages;

        R_target = build_target_response(fc, Q, gain_db, stages, freqs);

        err = evaluate_lut_row(rows(k).b, rows(k).a, R_target, w_grid);

        rows(k).rmse_db    = err.rmse_db;
        rows(k).wrmse_db   = err.wrmse_db;
        rows(k).max_abs_db = err.max_abs_db;

        % 这里默认把排序误差定义为 rmse_db
        rows(k).score = err.rmse_db;
    ...
    rows_sorted = rows(idx_sorted);

    % ============================================================
    % Plot
    % ============================================================
    figure('Name', 'IIR LUT Error Ranking', 'Color', 'w');
    plot(scores_sorted, 'LineWidth', 1.2);
    grid on;
    xlabel('Sorted rank');
    ylabel('RMSE (dB)');
    title('IIR LUT error ranking (descending)');

    hold on;
    plot(1, scores_sorted(1), 'ro', 'MarkerSize', 8, 'LineWidth', 1.5);
    text(1, scores_sorted(1), sprintf('  max = %.6f dB', scores_sorted(1)), ...
        'VerticalAlignment', 'bottom', 'Color', 'r');
    hold off;

    % ============================================================
    % Show worst case and whether it is an “extreme” point
    % ============================================================
    worst = rows_sorted(1);

    fprintf('\n============================================================\n');
    fprintf('Worst error row:\n');
    fprintf('idx      = [%d %d %d %d]\n', worst.idx(1), worst.idx(2), worst.idx(3), worst.idx(4));
    fprintf('fc       = %.16g Hz\n', worst.fc);
    fprintf('Q        = %.16g\n', worst.Q);
    fprintf('gain_db  = %.16g dB\n', worst.gain_db);
    fprintf('stages   = %.16g\n', worst.stages);
    fprintf('method   = %s\n', worst.method);
    fprintf('rmse_db  = %.9f dB\n', worst.rmse_db);
    fprintf('wrmse_db = %.9f dB\n', worst.wrmse_db);
    fprintf('max_abs  = %.9f dB\n', worst.max_abs_db);
    fprintf('extreme? = %s\n', explain_extreme(worst.idx));
    fprintf('============================================================\n\n');

    % 也打印前 10 个，方便看是不是边界/极端参数聚集
    topN = min(10, n);
    fprintf('Top-%d largest errors:\n', topN);
    for k = 1:topN
        r = rows_sorted(k);
        fprintf(['#%2d  err=%.6f dB | idx=[%2d %2d %2d %2d] | ' ...
                 'fc=%10.4f | Q=%9.5f | gain=%8.4f | stages=%6.4f | %s\n'], ...
                 k, r.score, r.idx(1), r.idx(2), r.idx(3), r.idx(4), ...
                 r.fc, r.Q, r.gain_db, r.stages, explain_extreme(r.idx));
    end

    % ============================================================
    % Export iirluts2.txt
    % only:
    % dims + prototype params + error + iir coeffs
    % ============================================================
    fprintf('\nWriting %s ...\n', out_file);
    ...
    cleanerOut = onCleanup(@() fclose(fid_out));

    for k = 1:n
        r = rows(k);
        fprintf(fid_out, ...
            '%d %d %d %d  %.16g %.16g %.16g %.16g  %.12e  %.18e %.18e %.18e %.18e %.18e  %.18e %.18e %.18e %.18e %.18e\n', ...
            r.idx(1), r.idx(2), r.idx(3), r.idx(4), ...
            r.fc, r.Q, r.gain_db, r.stages, ...
            r.score, ...
    ...
    % local helpers
    % ============================================================
    function R_target = build_target_response(fc, Q, gain_db, stages, freqs_local)
        wc = 2 * pi * fc;
        s_ = 1j * 2 * pi * freqs_local;

        A_target = 10^(gain_db / 20);

        Hs_peaking = @(ss) (1 + (A_target - 1) * ...
            (1 ./ (1 + (abs((imag(ss).^2 - wc^2) ./ (wc/Q * imag(ss) + 1e-200))).^(2*stages))));

        Hs_obj = Hs_peaking(s_);
        mag_target = abs(Hs_obj);
        R_target = mag_target.^2;
    end

    function out = evaluate_lut_row(b, a, R_target, w_grid_local)
        mag_target = sqrt(max(R_target, 0));
        h = freqz(b, a, w_grid_local);
        mag = abs(h);
        ...
        w_vis = w_vis(:);
        w_vis = w_vis / mean(w_vis);

        out.rmse_db    = sqrt(mean(err_db.^2));
        out.wrmse_db   = sqrt(sum(w_vis .* (err_db(:).^2)) / sum(w_vis));
        out.max_abs_db = max(abs(err_db));
    end

    function s = explain_extreme(idx4)
        labels = strings(1,4);
        for ii = 1:4
            if idx4(ii) == 1
                labels(ii) = "min";
        ...
        s = sprintf('f0=%s, Q=%s, gain=%s, stages=%s', ...
