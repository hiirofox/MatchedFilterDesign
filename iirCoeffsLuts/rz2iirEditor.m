...
function rz2iirGenerator()
    ...
    % ---- 方法 4: specirls + stmcb ----
    design_fn = @(Rt, wg, ord, fs_) design_matched_iir_specirls_stmcb(Rt, wg, ord);
    [b2c, a2c, aopt2c, winfo] = design_with_warped_target(design_fn, R_target, w_grid, order, fs, 'specirls-stmcb');
    warp_infos(end+1) = winfo;
    methods{end+1} = sprintf('specirls-stmcb (a=%.4f)', aopt2c);
    results(end+1) = evaluate_method(methods{end}, b2c, a2c, R_target, w_grid, fs);
...
function [b, a] = design_matched_iir_specirls_stmcb(R_target, w_grid, order)
    mag_target = sqrt(max(R_target, 0));
    ...
    % 用 specirls 结果做初始化，再 stmcb 迭代细化
    [b0, a0] = design_matched_iir_spectral(R_target,w_grid,order);
    [b, a] = stmcb(h_trunc, order, order, 20, a0);
